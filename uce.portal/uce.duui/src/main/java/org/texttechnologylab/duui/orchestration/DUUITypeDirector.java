...
public final class DUUITypeDirector implements DUUIDirector {
    ...
    public Optional<DUUICheckpoint<?>> checkpointFor(DUUIPipeline pipeline, DUUIArtifact<?> artifact) {
        ...
        return pipeline.checkpoints().stream()
                .filter(checkpoint -> checkpoint.artifactType().equals(artifact.artifactType())
                        || (checkpoint.payloadType() != null
                        && artifact.payloadType() != null
                        && checkpoint.payloadType().isAssignableFrom(artifact.payloadType())))
                .findFirst();
    }
}
