...
public class DUUIPipelineActorScopeTest extends TestCase {
    ...
    public void testGeneratorAdapterForkLambdaTargetBuilderFlowRunsEndToEnd() {
        List<UCEDocument> persisted = new ArrayList<>();
        List<UCECorpus> finalized = new ArrayList<>();

        try (DUUIPipelineScope pipeline = DUUIPipelineScope.open("uce-import")) {
            try (DUUIGeneratorScope<UCEImport> imports =
                         ImportReader.builder().root("input").open(pipeline)) {
                try (DUUIAdapterScope<UCEImport, UCECorpus> corpora =
                             ImportToCorpus.builder().open(imports)) {
                    try (DUUIForkScope<UCECorpus, UCEDocument> documents =
                                 DocumentReader.builder().open(corpora)) {
            ...
            DUUIOrchestrationResult result = new DUUIOrchestrator(pipeline.build()).run();

            assertEquals(0, result.unroutableArtifacts().size());
            assertEquals(2, persisted.size());
            assertEquals("doc-1", persisted.get(0).id());
            assertEquals("doc-2", persisted.get(1).id());
            assertEquals(1, finalized.size());
            assertEquals("input", finalized.get(0).id());
        }
    }
    ...
    private static final class ImportReader implements DUUIGenerator<UCEImport> {
        ...
        public void generate(DUUIArtifactEmitter<UCEImport> emitter) throws Exception {
            emitter.emit(artifact(new UCEImport(root)));
        }
    ...
    private static final class ImportToCorpus implements DUUIAdapter<UCEImport, UCECorpus> {
        ...
        static final class Builder {
            DUUIAdapterScope<UCEImport, UCECorpus> open(DUUIFlowScope<UCEImport> parent) {
                return new ImportToCorpus().open(parent);
            }
        }
    }
    ...
    private static final class DocumentReader implements DUUIFork<UCECorpus, UCEDocument> {
        ...
        static final class Builder {
            DUUIForkScope<UCECorpus, UCEDocument> open(DUUIFlowScope<UCECorpus> parent) {
                return new DocumentReader().open(parent);
            }
        }
    }
    ...
    private static final class PersistDocument implements DUUILambda<UCEDocument> {
        ...
        @Override
        public DUUIArtifact<UCEDocument> process(DUUIArtifact<UCEDocument> artifact) throws Exception {
            if (!artifact.payload().loaded()) {
                throw new IllegalStateException("Document was not loaded.");
            }
